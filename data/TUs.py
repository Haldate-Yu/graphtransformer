...
from data.LegacyTUDataset import LegacyTUDataset
...
def get_all_split_idx(dataset):
    ...
    root_idx_dir = '../data/' + dataset.name + '/'
...
class TUsDataset(torch.utils.data.Dataset):
    def __init__(self, name, pos_enc_dim=10):
        t0 = time.time()
        self.name = name

        # readout
        # dataset = TUDataset(self.name)
        dataset = LegacyTUDataset(self.name, min_allow_node=pos_enc_dim, hidden_size=1)  # dgl 4.0
        dataset.remove_small_graphs()
    ...
    def _make_full_graph(self):
        ...
        for split_num in range(10):
            self.train[split_num].graph_lists = [make_full_graph(g) for g in self.train[split_num].graph_lists]
            self.val[split_num].graph_lists = [make_full_graph(g) for g in self.val[split_num].graph_lists]
            self.test[split_num].graph_lists = [make_full_graph(g) for g in self.test[split_num].graph_lists]
    ...
    def _add_laplacian_positional_encodings(self, pos_enc_dim):
        ...
        # Graph positional encoding v/ Laplacian eigenvectors
        for split_num in range(10):
            self.train[split_num].graph_lists = [laplacian_positional_encoding(g, pos_enc_dim) for g in
                                                 self.train[split_num].graph_lists]
            self.val[split_num].graph_lists = [laplacian_positional_encoding(g, pos_enc_dim) for g in
                                               self.val[split_num].graph_lists]
            self.test[split_num].graph_lists = [laplacian_positional_encoding(g, pos_enc_dim) for g in
    ...
    def _add_wl_positional_encodings(self):
        ...
        for split_num in range(10):
            self.train[split_num].graph_lists = [wl_positional_encoding(g) for g in self.train[split_num].graph_lists]
            self.val[split_num].graph_lists = [wl_positional_encoding(g) for g in self.val[split_num].graph_lists]
            self.test[split_num].graph_lists = [wl_positional_encoding(g) for g in self.test[split_num].graph_lists]
